...
```
## Другие полезыне команды
- `dbt seed` - загружает данные в таблицы из CSV-файлов
- `dbt build` - основная команда, комбинирует run, test и seed (warning: dbt build не проверяет свежесть источников. Поэтому на практике обычно сперва запускают dbt source freshness, а следом — build/run/seed/etc)
